...
# ── Tests ─────────────────────────────────────────────────────────────────────
class TestScene:
    def test_home_200(self,client): assert client.get("/").status_code==200
    def test_is_html(self,client): assert "text/html" in client.get("/").content_type
    def test_has_panda_svg(self,client): assert "panda-svg" in client.get("/").data.decode()
    def test_has_river(self,client): assert "river" in client.get("/").data.decode()
    def test_has_balloons(self,client): assert "balloons-layer" in client.get("/").data.decode()
    def test_has_toggle(self,client): assert "toggleNight" in client.get("/").data.decode()
    ...
    def test_404(self,client): assert client.get("/nope").status_code==404

import pytest
@pytest.fixture()
def _app(): return create_app(testing=False)
@pytest.fixture()
