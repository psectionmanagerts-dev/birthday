...
function giftBoxPop(popX, popY){
  ...
  /* ── Stage 1: real collectible gift falls from pop-point to ground ─
     Uses JS rAF so we control timing precisely.                       */
  const landY  = window.innerHeight - 110;      // where it settles
  const startY = Math.max(popY, 60);            // starts at balloon position
  const fallDur= 1800;                          // ms — long enough to see clearly
  ...
  function animateFall(ts){
    ...
    // ease-in so it accelerates like real gravity
    const eased=prog*prog;
    const cy=startY+(landY-startY)*eased;
    const scale=.5+.5*prog;
    const rot=-15+15*prog;
    const opacity=Math.min(prog*4,1);
    fg.style.top=cy+'px';
    fg.style.opacity=opacity;
    fg.style.transform=`scale(${scale}) rotate(${rot}deg)`;

    /* ── Stage 1b: 🤫 shush emoji appears mid-fall at ~60% of journey ── */
    if(prog>=0.60 && !fg.dataset.shhShown){
      fg.dataset.shhShown='1';
      const shh=document.createElement('div');
      shh.textContent='🤫';
      shh.style.cssText=`position:absolute;left:${popX+18}px;top:${cy-40}px;
        font-size:26px;pointer-events:none;z-index:200;opacity:1;
        transition:transform 1.4s ease-out, opacity 1.4s ease-out;
        transform:translateY(0) scale(1)`;
      document.body.appendChild(shh);
      // float upward and fade
      requestAnimationFrame(()=>{
        shh.style.transform='translateY(-60px) scale(.8)';
        shh.style.opacity='0';
      });
      setTimeout(()=>shh.remove(),1500);
    }

    if(prog<1){ requestAnimationFrame(animateFall); return; }

    /* ── Stage 2: gift has LANDED — bounce settle then start idle bob ── */
    fg.dataset.landed='1';
    fg.style.transform='scale(1.15) rotate(0deg)';   // bounce squash
    ...
    // start idle bob CSS animation after settling
    setTimeout(()=>{
      fg.style.transition='';
      fg.style.animation='giftIdleBob 1.8s ease-in-out infinite';
      fg.style.filter='drop-shadow(0 3px 8px rgba(0,0,0,.22))';
      fg.style.pointerEvents='auto';  // now clickable after fully settled
      fg.style.cursor='pointer';
      // make it clickable now that it's settled
      ...
      /* ── Stage 3: quote card auto-reveals above the settled gift ────
         300ms after landing — gift is still, card pops up above it.   */
      setTimeout(()=>{
        if(fg.isConnected && fg.dataset.opened==='0'){
          revealQuote(popX, landY - 20);
        }
      }, 300);
    }, 420);

    // auto-fade if uncollected after 14s
    setTimeout(()=>{
...
/* ── Opening a fallen gift when clicked: pop animation ─────────── */
function openFallenGift(fg){
  const r=fg.getBoundingClientRect();
  const cx=r.left+r.width/2, cy=r.top+r.height/2;

  fg.style.animation='none';
  fg.style.transition='transform .3s ease, opacity .3s ease';
  fg.style.transform='scale(1.5) rotate(-10deg)';
  setTimeout(()=>{ fg.style.opacity='0'; fg.style.transform='scale(.2) rotate(25deg)'; },260);
  setTimeout(()=>fg.remove(),600);

  burst(cx,cy,'✨',10);
  confettiBurst(cx,cy,14);
}
...
/* ── 28 unique birthday quotes — one revealed per gift opened ──── */
