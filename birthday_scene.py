...
function giftBoxPop(x,y){
  ...
  // shush emoji — a little "secret surprise" wink right as the gift appears
  const shh=document.createElement('div');
  shh.textContent='🤫';
  shh.style.cssText=`position:absolute;left:${x+18}px;top:${y-40}px;
    font-size:26px;pointer-events:none;z-index:122;
    animation:giftPop 1.6s ease-out forwards;animation-delay:.15s`;
  document.body.appendChild(shh);
  setTimeout(()=>shh.remove(),1800);
