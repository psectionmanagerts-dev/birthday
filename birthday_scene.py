...
.flower.flaring{
  filter:drop-shadow(0 0 10px gold) drop-shadow(0 0 18px #fff89a) brightness(1.4);
  transform:scale(1.6) !important;
  animation:none !important;
}
.flower.settling{
  transition:transform 1.2s cubic-bezier(.34,1.56,.64,1), filter 1.2s;
}
...
.balloon-string{width:1.4px;margin:0 auto;transform-origin:top;
  animation:stringSway ease-in-out infinite, stringShimmer 1.6s ease-in-out infinite;
  background:linear-gradient(180deg,rgba(255,255,255,.7),rgba(120,90,60,.5))}
@keyframes stringSway{0%,100%{transform:skewX(-6deg)}50%{transform:skewX(6deg)}}
@keyframes stringShimmer{0%,100%{filter:brightness(1)}50%{filter:brightness(1.6)}}
...
/* ── Gift box burst ────────────────────────────────── */
.gift-box{position:absolute;pointer-events:none;z-index:120;font-size:30px;
  animation:giftPop 1.1s cubic-bezier(.34,1.56,.64,1) forwards}
@keyframes giftPop{
  0%{transform:translateY(0) scale(.2) rotate(0deg);opacity:0}
  35%{transform:translateY(-30px) scale(1.3) rotate(-10deg);opacity:1}
  70%{transform:translateY(-46px) scale(1) rotate(8deg);opacity:1}
  100%{transform:translateY(-70px) scale(.7) rotate(0deg);opacity:0}
}
...
function drawCanvas(dt){
  ...
  nightProgress += (target-nightProgress)*0.02;
...
const CYCLE_MS = 75000; // auto toggle every 75s
...
<svg id="teddy-svg" width="150" height="190" viewBox="0 0 150 190">
  <defs>
    <radialGradient id="furGrad" cx="40%" cy="30%" r="75%">
      ...
      <stop offset="100%" stop-color="#b97f45"/>
    </radialGradient>
  ...
  <!-- body -->
  <ellipse cx="75" cy="138" rx="50" ry="48" fill="url(#furGrad)" stroke="#7a5430" stroke-width="3"/>
  <ellipse cx="75" cy="146" rx="27" ry="25" fill="url(#furGradLight)"/>

  <!-- legs -->
  <ellipse cx="52" cy="178" rx="17" ry="12" fill="url(#furGrad)" stroke="#7a5430" stroke-width="2.5"/>
  <ellipse cx="98" cy="178" rx="17" ry="12" fill="url(#furGrad)" stroke="#7a5430" stroke-width="2.5"/>

  <!-- left arm -->
  <g id="larm" style="transform-origin:32px 128px">
    <rect x="20" y="118" width="22" height="38" rx="11" fill="url(#furGrad)" stroke="#7a5430" stroke-width="2.5" transform="rotate(18,31,128)"/>
  </g>
  <!-- right arm (holds item depending on state) -->
  <g id="rarm" style="transform-origin:118px 128px">
    <rect x="106" y="116" width="22" height="38" rx="11" fill="url(#furGrad)" stroke="#7a5430" stroke-width="2.5" transform="rotate(-18,117,126)"/>
  </g>

  <!-- head -->
  <ellipse cx="75" cy="70" rx="52" ry="50" fill="url(#furGrad)" stroke="#7a5430" stroke-width="3"/>
  <!-- ears -->
  <circle cx="30" cy="28" r="19" fill="url(#furGrad)" stroke="#7a5430" stroke-width="2.5"/>
  <circle cx="120" cy="28" r="19" fill="url(#furGrad)" stroke="#7a5430" stroke-width="2.5"/>
  <circle cx="30" cy="28" r="9" fill="#caa074"/>
  <circle cx="120" cy="28" r="9" fill="#caa074"/>
  <!-- muzzle -->
...
function setVisualForState(state){
  ...
  document.getElementById('eyes').style.display = state==='sleeping' ? 'none':'block';
...
function giftBoxPop(x,y){
  ...
  g.textContent='🎁';
  g.style.cssText=`left:${x-15}px;top:${y-15}px`;
