...
.falling-gift{
  ...
  z-index:60;
  font-size:32px;
  animation:giftFall 2s ease-out forwards, giftIdleBob 1.8s ease-in-out infinite 2s;
  filter:drop-shadow(0 3px 6px rgba(0,0,0,.25));
  transition:transform .2s ease, filter .2s ease;
}
.falling-gift:hover{
  transform:scale(1.15) !important;
  filter:drop-shadow(0 0 10px rgba(255,210,120,.9)) drop-shadow(0 3px 6px rgba(0,0,0,.25));
}
@keyframes giftFall{
  0%{
...
function giftBoxPop(x,y){
  ...
  // a real collectible gift drifts down to the ground — click it to open!
  ...
  // a fallen gift left unopened for too long fades away gracefully
  setTimeout(()=>{
    if(fg.dataset.opened==='0' && fg.isConnected){
      fg.style.transition='opacity 1.2s ease, transform 1.2s ease';
      fg.style.opacity='0';
      fg.style.transform='scale(.6)';
      setTimeout(()=>fg.remove(),1300);
    }
  }, 14000);
}

/* ── Opening a fallen gift: shush emoji + a unique birthday quote ── */
function openFallenGift(fg){
  const r=fg.getBoundingClientRect();
  const cx=r.left+r.width/2, cy=r.top+r.height/2;

  // the gift box "pops open"
  fg.style.transition='transform .3s ease, opacity .3s ease';
  fg.style.transform='scale(1.4) rotate(-8deg)';
  setTimeout(()=>{ fg.style.opacity='0'; fg.style.transform='scale(.3) rotate(20deg)'; },280);
  setTimeout(()=>fg.remove(),650);

  // shush emoji — the secret surprise moment, right as it's opened
  const shh=document.createElement('div');
  shh.textContent='🤫';
  shh.style.cssText=`position:absolute;left:${cx+16}px;top:${cy-44}px;
    font-size:28px;pointer-events:none;z-index:122;
    animation:giftPop 1.6s ease-out forwards`;
  document.body.appendChild(shh);
  setTimeout(()=>shh.remove(),1700);

  // sparkle flourish around the opened gift
  burst(cx,cy,'✨',8);

  // reveal one of the 28 unique birthday quotes
  revealQuote(cx, cy-60);
}
  ...
  function popBalloon(viaClick, clientX, clientY){
    ...
    if(viaClick){
      ...
      if(pops===14 && !nightTriggeredAt14){
        ...
        setTimeout(()=>{ if(window.toggleNight) window.toggleNight(false); }, 400);
      }
