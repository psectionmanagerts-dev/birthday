...
#teddy.walking{animation:teddyBob .55s ease-in-out infinite}
@keyframes teddyBob{0%,100%{transform:translateY(0) rotate(-3deg)}50%{transform:translateY(-11px) rotate(3deg)}}
#teddy.walking #leg-left{animation:legStepL .55s ease-in-out infinite}
#teddy.walking #leg-right{animation:legStepR .55s ease-in-out infinite}
#teddy.walking #larm{animation:armSwingL .55s ease-in-out infinite}
#teddy.walking #rarm{animation:armSwingR .55s ease-in-out infinite}
@keyframes legStepL{0%,100%{transform:translateY(0) rotate(-22deg)}50%{transform:translateY(-5px) rotate(22deg)}}
@keyframes legStepR{0%,100%{transform:translateY(-5px) rotate(22deg)}50%{transform:translateY(0) rotate(-22deg)}}
@keyframes armSwingL{0%,100%{transform:rotate(18deg)}50%{transform:rotate(-18deg)}}
@keyframes armSwingR{0%,100%{transform:rotate(-18deg)}50%{transform:rotate(18deg)}}
...
/* ── Intro title card ──────────────────────────────────── */
#intro-card{position:fixed;inset:0;z-index:500;display:flex;flex-direction:column;
  align-items:center;justify-content:center;
  background:radial-gradient(ellipse at center,#1a003a 0%,#000 100%);
  animation:introFadeOut 1s ease-in-out 3.2s forwards}
@keyframes introFadeOut{0%{opacity:1;pointer-events:auto}100%{opacity:0;pointer-events:none}}
#intro-card h1{font-size:clamp(2.2rem,8vw,5rem);color:white;margin:0;letter-spacing:2px;
  text-shadow:0 0 30px #ff88cc,0 0 60px #aa44ff;
  animation:introTitle 3.2s ease-out forwards}
@keyframes introTitle{
  0%{opacity:0;transform:scale(.4) translateY(40px)}
  35%{opacity:1;transform:scale(1.06) translateY(-6px)}
  55%{transform:scale(.98) translateY(0)}
  80%,100%{opacity:1;transform:scale(1)}
}
#intro-card p{font-size:clamp(1rem,3vw,1.6rem);color:#ffccee;margin-top:.6rem;
  animation:introSub 3.2s ease-out .4s forwards;opacity:0}
@keyframes introSub{0%{opacity:0;transform:translateY(20px)}40%{opacity:1;transform:translateY(0)}100%{opacity:1}}
.intro-star{position:absolute;font-size:1.4rem;
  animation:introStarFloat ease-in-out infinite}
...
<!-- Intro title card — dissolves after 3.2s -->
<div id="intro-card">
  <span class="intro-star" style="left:8%;top:15%;animation-duration:2.2s;animation-delay:0s">✨</span>
  <span class="intro-star" style="left:88%;top:12%;animation-duration:1.8s;animation-delay:.4s">🌟</span>
  <span class="intro-star" style="left:15%;top:75%;animation-duration:2.5s;animation-delay:.2s">💫</span>
  <span class="intro-star" style="left:80%;top:70%;animation-duration:2s;animation-delay:.6s">⭐</span>
  <span class="intro-star" style="left:50%;top:10%;animation-duration:1.9s;animation-delay:.1s">🎉</span>
  <h1>🎂 Happy Birthday!</h1>
  <p>Pop the balloons to reveal the magic ✨</p>
</div>
...
teddy.id='teddy';
teddy.innerHTML=`
<svg id="teddy-svg" width="160" height="200" viewBox="0 0 160 200">
  <defs>
    <radialGradient id="furGrad" cx="38%" cy="28%" r="80%">
    ...
    <radialGradient id="earInner" cx="45%" cy="40%" r="65%">
  ...
  <!-- ground shadow -->
  <ellipse cx="80" cy="195" rx="46" ry="7" fill="rgba(0,0,0,.16)"/>

  <!-- ═══ legs (own groups for step animation) ═══ -->
  <g id="leg-left" style="transform-origin:58px 178px">
    <path d="M44,150 Q40,178 50,190 Q58,196 68,190 Q74,180 70,158 Q62,148 44,150 Z"
      fill="url(#furGrad)" stroke="#c9c9c9" stroke-width="2"/>
    <ellipse cx="56" cy="189" rx="13" ry="7" fill="#fdfdfd" stroke="#d8d8d8" stroke-width="1.5"/>
  </g>
  <g id="leg-right" style="transform-origin:102px 178px">
    <path d="M116,150 Q120,178 110,190 Q102,196 92,190 Q86,180 90,158 Q98,148 116,150 Z"
      fill="url(#furGrad)" stroke="#c9c9c9" stroke-width="2"/>
    <ellipse cx="104" cy="189" rx="13" ry="7" fill="#fdfdfd" stroke="#d8d8d8" stroke-width="1.5"/>
  </g>

  <!-- ═══ body: soft rounded pear shape, single cohesive path ═══ -->
  <path d="M80,98
    C108,98 124,118 122,146
    C120,170 102,184 80,184
    C58,184 40,170 38,146
    C36,118 52,98 80,98 Z"
    fill="url(#furGrad)" stroke="#c9c9c9" stroke-width="2.5"/>
  <!-- belly patch -->
  <ellipse cx="80" cy="148" rx="26" ry="28" fill="url(#furGradLight)"/>

  <!-- ═══ arms ═══ -->
  <g id="larm" style="transform-origin:42px 122px">
    <path d="M42,108 C28,110 18,124 20,140 C21,150 30,156 38,152 C44,148 46,136 44,122 Z"
      fill="url(#furGrad)" stroke="#c9c9c9" stroke-width="2.2"/>
  </g>
  <g id="rarm" style="transform-origin:118px 120px">
    <path d="M118,106 C132,108 142,122 140,138 C139,148 130,154 122,150 C116,146 114,134 116,120 Z"
      fill="url(#furGrad)" stroke="#c9c9c9" stroke-width="2.2"/>
  </g>

  <!-- ═══ head: round with a gentle neck blend into the body ═══ -->
  <path d="M80,18
    C108,18 128,38 128,66
    C128,90 112,108 96,110
    L64,110
    C48,108 32,90 32,66
    C32,38 52,18 80,18 Z"
    fill="url(#furGrad)" stroke="#c9c9c9" stroke-width="2.5"/>

  <!-- ears: nestled into the head's silhouette, not floating off it -->
  <circle cx="40" cy="28" r="18" fill="url(#furGrad)" stroke="#c9c9c9" stroke-width="2.2"/>
  <circle cx="120" cy="28" r="18" fill="url(#furGrad)" stroke="#c9c9c9" stroke-width="2.2"/>
  <circle cx="40" cy="28" r="9" fill="url(#earInner)"/>
  <circle cx="120" cy="28" r="9" fill="url(#earInner)"/>

  <!-- muzzle: blended oval, soft edge fading into the face -->
  <ellipse cx="80" cy="78" rx="25" ry="19" fill="url(#furGradLight)"/>

  <!-- eyes group -->
  <g id="eyes">
    <circle cx="62" cy="58" r="6" fill="#3a2a1e"/>
    <circle cx="98" cy="58" r="6" fill="#3a2a1e"/>
    <circle cx="64" cy="55.5" r="1.8" fill="white"/>
    <circle cx="100" cy="55.5" r="1.8" fill="white"/>
  </g>
  <!-- closed eyes (sleeping) -->
  <g id="eyes-closed" style="display:none">
    <path d="M55,58 Q62,63 69,58" fill="none" stroke="#3a2a1e" stroke-width="2.4" stroke-linecap="round"/>
    <path d="M91,58 Q98,63 105,58" fill="none" stroke="#3a2a1e" stroke-width="2.4" stroke-linecap="round"/>
  </g>
  <!-- downward eyes (watching laptop) -->
  <g id="eyes-down" style="display:none">
    <circle cx="62" cy="61" r="6" fill="#3a2a1e"/>
    <circle cx="98" cy="61" r="6" fill="#3a2a1e"/>
    <circle cx="62" cy="64" r="1.8" fill="white"/>
    <circle cx="98" cy="64" r="1.8" fill="white"/>
  </g>

  <!-- eyebrow hints for warmth -->
  <path d="M54,48 Q62,44 70,47" fill="none" stroke="#d8d8d8" stroke-width="2" stroke-linecap="round" opacity=".7"/>
  <path d="M90,47 Q98,44 106,48" fill="none" stroke="#d8d8d8" stroke-width="2" stroke-linecap="round" opacity=".7"/>

  <!-- cheeks -->
  <ellipse cx="48" cy="74" rx="11" ry="8" fill="#ffaebd" opacity=".5"/>
  <ellipse cx="112" cy="74" rx="11" ry="8" fill="#ffaebd" opacity=".5"/>

  <!-- nose -->
  <path d="M73,68 Q80,64 87,68 Q86,75 80,76 Q74,75 73,68 Z" fill="#3a2a1e"/>

  <!-- mouth -->
  <path id="mouth" d="M68,80 Q80,90 92,80" fill="none" stroke="#3a2a1e" stroke-width="2.4" stroke-linecap="round"/>
</svg>

<!-- Lollipop (visible in walking/idle states) -->
<div id="teddy-lollipop" style="position:absolute;right:-14px;top:6px;z-index:30">
...
<div id="teddy-laptop" style="position:absolute;left:20px;bottom:16px;display:none;z-index:30">
...
<div id="teddy-food" style="position:absolute;left:8px;bottom:8px;display:none;z-index:30;font-size:30px">🍰</div>

<!-- Zzz for sleeping -->
<div id="teddy-zzz" style="position:absolute;right:8px;top:-8px;display:none">
  <span class="zzz" style="left:0px">💤</span>
</div>

<!-- Crown — appears during finale -->
...
function giftBoxPop(popX, popY){
  ...
  /* ── Stage 1: gift falls from balloon position to ground ──────────
     Clickable the entire time — during fall AND after landing.
     Click at any point → shush emoji + quote. Nothing auto-fires.  */
  ...
  /* Single click handler — works whether gift is mid-fall or settled */
  ...
  function animateFall(ts){
    if(fg.dataset.opened==='1') return; // clicked during fall — stop animating
    if(!t0) t0=ts;
    const prog=Math.min((ts-t0)/fallDur,1);
    const eased=prog*prog;                        // gravity acceleration
    ...
    // sprinkle trail particles during fall (every ~6th frame)
    ...
    /* Gift has landed — bounce squash, then idle bob */
    fg.dataset.landed='1';
    ...
    setTimeout(()=>{
      ...
      fg.style.transition='transform .18s ease-in';
      setTimeout(()=>{
        fg.style.transition='';
...
/* ── Click on gift (falling OR settled): shush → gift pops → quote ─
   Exact sequence:
     0ms   gift bounces/pops open with rotate+scale animation
    80ms   🤫 shush emoji floats up beside it
   700ms   quote card reveals above where the gift was             ── */
function openFallenGift(fg){
  ...
  fg.style.transform='scale(1.5) rotate(-12deg)';
  setTimeout(()=>{
    fg.style.opacity='0';
    fg.style.transform='scale(.15) rotate(25deg)';
  }, 260);
  setTimeout(()=>fg.isConnected && fg.remove(), 580);

  /* Sparkle burst at open point */
  burst(cx, cy, '✨', 8);

  /* 🤫 shush emoji rises up beside the opening gift */
  setTimeout(()=>{
    const shh=document.createElement('div');
    shh.textContent='🤫';
    shh.style.cssText=`position:fixed;left:${cx+20}px;top:${cy-10}px;
      font-size:32px;pointer-events:none;z-index:220;
      transition:transform 1.2s ease-out, opacity 1.2s ease-out`;
    document.body.appendChild(shh);
    requestAnimationFrame(()=>{
      shh.style.transform='translateY(-65px) scale(.85)';
      shh.style.opacity='0';
    });
    setTimeout(()=>shh.remove(), 1300);
  }, 80);

  /* Quote card pops up above gift position — only after shush has appeared */
  setTimeout(()=>{
    revealQuote(cx, cy - 55);
  }, 700);
}
